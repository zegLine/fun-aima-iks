 ...
 */
public class ImprovedLRTAStarAgent<P, S, A> extends SimpleAgent<P, A> {
	 ...
	 */
	public ImprovedLRTAStarAgent(OnlineSearchProblem<S, A> problem, Function<P, S> ptsFn, ToDoubleFunction<S> h) {
		setProblem(problem);
	...
	public Optional<A> act(P psPrimed) {
		...
		if (problem.testGoal(sPrimed)) {
			...
			/// a <- an action b in ACTIONS(s') that minimizes LRTA*-COST(s', b, result[s', b], H)
			/// Improvement: avoid the action that leads straight back to s (the previous location),
			/// unless it is the only option available (dead end).
			List<A> candidates = problem.getActions(sPrimed);
			Comparator<A> byCost = Comparator.comparingDouble(b -> lrtaCost(sPrimed, b, result.get(sPrimed, b)));
			a = candidates.stream()
					.filter(b -> !isItActuallyGoingBack(b, s))
					.min(byCost)
					.orElseGet(() -> candidates.stream().min(byCost).orElse(null));
		}
		if (a == null)
	...
	/// this returns true if the action would result in the previous state, as in a 180 degree turn
	private boolean isItActuallyGoingBack(A action, S previous) {
		return previous != null
				&& action instanceof MoveToAction
				&& ((MoveToAction) action).getToLocation().equals(previous);
	}
